  ...
  methods: {
    ...
    async fetchRoutes() {
      try {
        ...
        const query = `
          [out:json];
          way[highway~"^(motorway|trunk|primary|secondary|tertiary)$"](around:450,${this.semarangCoordinates[0]},${this.semarangCoordinates[1]});
          out;>;out skel;
        `;
    ...
    getWayRute(start, end, rute){
      ...
      let hasilRute = this.getRute(wayRute, nodeAwal, end, [])
    ...
    drawRoutes(data) {
      ...
      // const way = ways[6]
        ...
        for (let i = 0; i < latlngs.length; i++) {
          ...
          // L.marker(latlng).addTo(this.map).bindPopup(`${i} ${way.id} ${way.tags.name} ${way.tags.oneway}`)
        }
      });
    },
  },
