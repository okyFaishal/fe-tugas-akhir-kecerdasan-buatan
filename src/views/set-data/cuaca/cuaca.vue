<template>
  <div class="card">
    ...
    <div class="flex gap-3">
      ...
      <Button label="set banjir" class="p-3" severity="primary" @click="setBanjir()"/>
    </div>
  </div>
  <div class="card">
  ...
  methods: {
    ...
    async setBanjir(){
      ...
      try {
        ...
        // console.log('res', res)
        if(res.data.status == 200){
          vm.$toast.add({ severity: 'success', summary: 'Konfirmasi', detail: 'Berhasil set data banjir', life: 3000 });
          vm.resetArea();
          vm.getData()
        }else{
          vm.$toast.add({ severity: 'error', summary: 'Error', detail: res.data.message, life: 3000 });
