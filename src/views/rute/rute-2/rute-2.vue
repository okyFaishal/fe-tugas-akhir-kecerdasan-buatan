    ...
    async onMapClick(e) {
      ...
      if (!vm.dataForm.asal) {
        ...
        if(res.data.status == 200){
          ...
          console.log("res.data.data", JSON.parse(res.data.data.replace(/'/g, '"').replace(/False/g, 'false').replace(/True/g, 'true')))
          vm.dataForm.listRute.push(L.polyline(JSON.parse(res.data.data.replace(/'/g, '"').replace(/False/g, 'false').replace(/True/g, 'true')).map(x => [x.lat_rute_koordinat, x.lng_rute_koordinat]), { color: "blue" }).addTo(vm.map)); // rute garis
        }
      }
